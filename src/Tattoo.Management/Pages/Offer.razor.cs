...
namespace Tattoo.Management.Pages
{
    ...
    public partial class Offer
    {
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            ///
            //offerValues = Enum.GetValues(typeof(CacheValueTypeWithDescript))
            //              .Cast<CacheValueTypeWithDescript>()
            //              .Where(e => e.ToString().StartsWith("Offer"))
            //              .Select(e => new KeyValuePair<int, string>((int)e, e.ToString()))
            //              .ToList();
        }

        //private List<KeyValuePair<int, string>> offerValues;
        /// <summary>
        ...
        HashSet<ConfigValueDto> _ConfigValuesDataFiltered = new();
        ...
        //private readonly CacheValueType _customValueType = CacheValueType.AskAnswerFAQEspannol;
        ...
        async Task OnChangeType(object type) 
        {
            _ConfigValuesDataFiltered = _ConfigValuesData.Where(x => x.ValueType == (CacheValueType)type).ToHashSet();
        }
        ...
        async Task DeleteRow(ConfigValueDto ConfigValue)
        {
            ...
            if (ConfigValueEdit != null)
            {
                ...
                bool hasSucess = await ConfirmDelete(ConfigValueEdit!.Id, ConfigValue.ValueType);
                if (!hasSucess) return;
        ...
        private async Task LoadConfigValues()
        {
            ...
            InvokeAsync(async () =>
            {
                ...
                if (response.IsSuccess)
                {
                    ...
                    _ConfigValuesDataFiltered = parsedResponse?.ToHashSet() ?? new();
                }
                else
                {
        ...
        protected async Task<bool> DeleteConfigValue(long id, CacheValueType configType)
        {
            var response = await _configValueService.DeleteConfigValueAsync(id, configType);
        ...
        protected async Task<bool> ConfirmDelete(long id, CacheValueType configType)
        {
            ...
            {
                return await DeleteConfigValue(id, configType);
            }
            return false;
        }
